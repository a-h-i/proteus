...
#include <cstring>
// 4 second ringBuffer
Prot::Prot( const std::string &grammar, const std::string &dict,
...
char *Prot::recogWord(const silencers::silencer_t &silen) {
    // first we instruct the mic to start recording
    // WE must empty the buffer
    auto dummy = [](std::int16_t s){return true;};
...
char * Prot::singleUttrHelper(const silencers::silencer_t &silen) {
    const char *utt, *hyp;
    hyp = utt = nullptr;
...
void Prot::buffer( const std::int16_t *buff, std::size_t size ) {
    // perfectly safe
